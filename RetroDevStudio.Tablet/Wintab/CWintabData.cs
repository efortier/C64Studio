...
namespace WintabDN
{
	...
	public class CWintabData
	{
		...
		public WintabPacket GetDataPacket(UInt32 hCtx_I, UInt32 pktID_I)
		{
			...
			// LOCAL FIX (RetroDevStudio): upstream WintabDN never frees buf here,
			// leaking sizeof(WintabPacket) of unmanaged heap on EVERY packet — and
			// this overload is on the pen hot path (~133 Hz). Free it like every
			// sibling method in this library does.
			try
			{
				if (pktID_I == 0)
				{
