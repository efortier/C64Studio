...
namespace RetroDevStudio.Documents
{
  public partial class ValueTableEditor : BaseDocument
  {
    ...
    public override bool LoadDocument()
    {
      ...
      {
        Core.Notification.MessageBox( "Could not load file", "Could not load value table project file " + DocumentInfo.FullPath + ".\r\n" + ex.Message );
        return false;
      }
      pictureGraphPreview.Invalidate();
      SetUnmodified();
    ...
    private void btnExportToBASICData_Click( DecentForms.ControlBase Sender )
    {
      ...
      editDataExport.Text = Util.ToBASICData( exportData, curLineNumber, lineDelta, 80, 0, checkInsertSpaces.Checked, -1 );
    }
    ...
    private void btnExportToBASICDataHex_Click( DecentForms.ControlBase Sender )
    {
      ...
      int         lineDelta = GR.Convert.ToI32( editExportBASICLineOffset.Text );
      int         curLineNumber = GR.Convert.ToI32( editExportBASICLineNo.Text );
      editDataExport.Text = Util.ToBASICHexData( exportData, curLineNumber, lineDelta, checkInsertSpaces.Checked, -1 );
    }
