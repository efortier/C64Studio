...
namespace RetroDevStudio.Documents
{
  public partial class DebugMemory : BaseDocument
  {
    ...
    public void MarkAllMemoryAsUnknown()
    {
      for ( int i = 0; i < Core.Debugging.ActiveMemory.Flags.Length; ++i )
      {
        Core.Debugging.ActiveMemory.Flags[i] = 0;
      }
    }
    ...
    public void InvalidateAllMemory()
    {
      for ( int i = 0; i < Core.Debugging.ActiveMemory.Flags.Length; ++i )
      {
        Core.Debugging.ActiveMemory.Flags[i] &= ~MemoryView.RAMFlag.VALUE_KNOWN;
      }
    }
